...
to learn how to use this program to play Arimaa. We learned rules of Arimaa from 
...
Authors: Emily MacPherson, Oria Weng
Date: 1/31/21
*/
...
public class Arimaa implements MouseListener, ActionListener
{
  ...
  JButton instructionsButton = new JButton("Hide instructions");
	...
	Container allEliminatedContainer = new Container();
	Container goldEliminatedContainer = new Container();
	Container silverEliminatedContainer = new Container();
	Container eliminatedControl = new Container();
	JLabel eliminatedLBL = new JLabel("Eliminated pieces", SwingConstants.CENTER);
	JButton eliminatedButton = new JButton("Hide eliminated");
	/*bookmark*/
  ...
  public Arimaa()
  {
    ...
    JOptionText.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 15)); //for changing font of JOptionPane
  ...
  //starts the setup of the pieces on the board
  ...
  public void actionPerformed(ActionEvent event)
  {
		...
		if (event.getSource().equals(push)) 
    {
      ...
      endTurn.setEnabled(false);
		...
		//when end turn button is clicked, check to make sure the last move was valid (there was a net change
		//and not third time position has occurred); if it was, change the player state
		if (event.getSource().equals(endTurn))
    {    
			unColorLastSpot(previousPositionX, previousPositionY); //in case the player selected a piece to move
      ...
      {
        JOptionText.setText("<html>This play has existed more than twice.<br>Move a different direction or a different piece to change it.<html>");
		    JOptionPane.showMessageDialog(frame, JOptionText);
				revertToLastBoardPosition(); //undo the whole turn
		    movesRemaining = 4;
        movesRemainingLBL.setText("    " + movesRemaining);
				endTurn.setEnabled(false);
        
        if (playerState.equals("goldPlay") || playerState.equals("goldMove"))
        { 
      ...
      else //valid turn end
      {
		...
		//hide or show eliminated pieces according to current button text; change current button text
		if (event.getSource().equals(eliminatedButton))
		{
			if (eliminatedButton.getText().equals("Hide eliminated"))
			{
				eliminatedLBL.setVisible(false);
				goldEliminatedContainer.setVisible(false);
				silverEliminatedContainer.setVisible(false);
				eliminatedButton.setText("Show eliminated");
			}
			else
			{
				eliminatedLBL.setVisible(true);
				goldEliminatedContainer.setVisible(true);
				silverEliminatedContainer.setVisible(true);
				eliminatedButton.setText("Hide eliminated");
			}
		}

    //if the source is a gold setup button, switch to choosing a place to place it on the board
    ...
    //if the source is a silver setup button, switch to choosing a place to place it on the board
    ...
    //if the source is one of the buttons on the board, do according to the current state...
		for (int x = 0; x < 8; x++)
		{
			for (int y = 0; y < 8; y++)
			{
				...
				{
					//place the piece selected earlier on the board in the current board spot for setup state
          ...
          //the play state is when a piece is selected by the player to interact with (move, push, or pull)
          ...
          //the move state is when a square that the selected piece will move to is clicked
            ...
            if((x == 2 && y == 2) || (x == 2 && y == 5) || (x == 5 && y == 2) || (x == 5 && y == 5))
            {
              ...
              int input = JOptionPane.showConfirmDialog(frame, JOptionText, "Trap Square Warning",
          ...
          //the push place state is when the place that the weak piece will be pushed to is selected
					else if (playerState.equals("goldPushPlace"))
					{
						...
						movePiece(allPieces[previousPositionX][previousPositionY], previousPositionX, previousPositionY,
						weakX, weakY);
          ...
          //the pull piece state is when a piece that the selected piece will pull is clicked 
            ...
            weakX = x;
						weakY = y;
						enableSquaresMovableTo(allPieces[previousPositionX][previousPositionY], previousPositionX,
						previousPositionY);
          ...
          //the pull place state is when the place that the strong piece will move to in order to pull the
					//weak piece is clicked
  ...
  //called whenever the mouse clicks something, whether it is disabled or not
  public void mouseReleased(MouseEvent event)
  {
    ...
    if(SwingUtilities.isRightMouseButton(event) == true)
    {
      if(playerState.equals("goldSetup") && goldSetupButtons.contains(event.getSource()))
      {
        for (int a = 0; a < goldAnimals.size(); a++)
        {
          //if the background is colored that means it is selected so it can be de-selected.
          if(event.getSource().equals(goldSetupButtons.get(a)) &&
					goldSetupButtons.get(a).getBackground().equals(new Color(224, 214, 173)))
          {
      ...
      else if(playerState.equals("silverSetup") && silverSetupButtons.contains(event.getSource()))
      {
        ...
        {
          if(event.getSource().equals(silverSetupButtons.get(a)) &&
					silverSetupButtons.get(a).getBackground().equals(new Color(214, 214, 214)))
          {
            silverSetupButtons.get(a).setBackground(null);
            setupPieceSelected = "";
            enableSilverSetupButtons();
			...
			else if(event.getSource().equals(pull)) 
      {
        ...
        if(pull.getBackground().equals(new Color(224, 214, 173)))
        {
          ...
          pull.setEnabled(true);
          if(pushableOrPullablePiecesExist(playingPieceSelected, previousPositionX, previousPositionY,
					"push") == true)
          {
            push.setEnabled(true);
          }
      ...
      for(int x = 0; x < 8; x++)
      {
        ...
        {
          if(event.getSource().equals(board[x][y]) &&
					(board[x][y].getBackground().equals(new Color(224, 214, 173)) ||
					board[x][y].getBackground().equals(new Color(214, 214, 214))))
          {
            push.setEnabled(false);
            pull.setEnabled(false);
  ...
  //checks if a set up button should be removed (its number of pieces has reached 0)
  ...
  public void startGame()
  {
    ...
    north.add(instructionsButton);
		...
		south.setLayout(new FlowLayout());
		eliminatedControl.setLayout(new GridLayout(2, 1));
		eliminatedLBL.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 10));
    eliminatedLBL.setPreferredSize(new Dimension(100, 30));
		eliminatedLBL.setVerticalAlignment(SwingConstants.TOP);
		eliminatedControl.add(eliminatedLBL);
		eliminatedButton.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 9));
    eliminatedLBL.setPreferredSize(new Dimension(100, 30));
		eliminatedButton.addActionListener(this);
		eliminatedControl.add(eliminatedButton);
		south.add(eliminatedControl);
		allEliminatedContainer.setLayout(new GridLayout(2, 1));
		goldEliminatedContainer.setLayout(new FlowLayout());
		silverEliminatedContainer.setLayout(new FlowLayout());
		allEliminatedContainer.add(goldEliminatedContainer); //these are currently empty
		allEliminatedContainer.add(silverEliminatedContainer);
		south.add(allEliminatedContainer);
		/*bookmark*/
  ...
  //disables all gold/silver pieces that cannot move, either because a piece is frozen or all its sides are
	//blocked off by pieces it cannot push
  public void enableMovablePieces(String color)
  {
    ...
    for (int x = 0; x < 8; x++) 
    {
			for (int y = 0; y < 8; y++) 
      {
        ...
        if(allPieces[x][y] != null && allPieces[x][y].getColor().equals(color) &&
				isFrozen(allPieces[x][y]) == false)
        {
          //check if any of the pieces around the current piece are pushable. If so, the current piece
          ...
          else if(((x < 7 && x > 0) && (y > 0 && y < 7) && allPieces[x][y].getNeighbors().size() < 4) ||
					((x == 7 ^ x == 0 ^ y == 0 ^ y == 7) && allPieces[x][y].getNeighbors().size() < 3) ||
					(((x == 0 && y == 0) || (x == 7 && y == 0) || (x == 7 && y == 7) || (x == 0 && y == 7)) &&
					allPieces[x][y].getNeighbors().size() < 2)) //'^' is xor, which means only one of the conditions
					//may be true for the section to be true (https://www.baeldung.com/java-xor-operator)
          {
            board[x][y].setEnabled(true);
          }
        }
  ...
  //check the 4 cardinal directions of a given piece for other pieces; adds those pieces to an ArrayList of
	//neighbors
  ...
  //returns if a piece is frozen (if it has an adjacent more powerful enemy and does not have an adjacent
	//friendly piece)
  public boolean isFrozen(Piece currentPiece)
  {
    ...
    for(int a = 0; a < currentPiece.getNeighbors().size(); a++)
    {
			if(currentPiece.getNeighbors().get(a) != null)
			{
				...
				if(neighborPiece.getPowerLevel() > currentPiece.getPowerLevel() &&
				!neighborPiece.getColor().equals(currentPiece.getColor()))
				{
  ...
  public boolean pushableOrPullablePiecesExist(Piece currentPiece, int x, int y, String pushOrPull)
  {
	...
	//called after a player selects one of their own pieces and clicks the push or pull button, enables the
	//pieces the stronger piece can push/pull
  ...
  public void endGame()
  {
    frame.dispose(); //will close window
		...
		System.exit(0); //must stop program, otherwise multiple losing/winning pop-ups can appear
  }
  ...
  //called whenever gold completes a move (a movement of a piece, a push, or a pull)
    ...
    //if it's not over with
      ...
      //then we make sure this position of the board hasen't happened more than twice 
      else if(previouslyPlayed() == true) 
      {
        ...
        movesRemainingLBL.setText("    " + movesRemaining);
				endTurn.setEnabled(false);
        enableMovablePieces("gold"); //still Gold's turn
        playerState = "goldPlay";
      }
  ...
  //called whenever silver completes a move (a movement of a piece, a push, or a pull)
    ...
    //if it's not over with
      ...
      else if(previouslyPlayed() == true)  //this can't be the third time reaching this position with the
			//same piece
      {
        JOptionText.setText("<html>This play has existed more than twice.<br>Move a different direction or a different piece to change it.<html>");
		    JOptionPane.showMessageDialog(frame, JOptionText);
        ...
        movesRemainingLBL.setText("    " + movesRemaining);
				endTurn.setEnabled(false);
        enableMovablePieces("silver"); //still Silver's turn
        playerState = "silverPlay";
      }
      else
      {
  ...
  public void checkPieceCaptured()
  {
    for(int x = 0; x < 8; x++)
    {
      for(int y = 0; y < 8; y++)
      {
        if(board[x][y].getBackground().equals(new Color(255, 128, 128)) && allPieces[x][y] != null &&
				hasFriendlyNeighbors(allPieces[x][y], x, y) == false)
        {
          ...
          else if(allPieces[x][y].getColor().equals("silver"))
          {
            ...
            if(allPieces[x][y].getPowerLevel() == 1)
            {
              silverRabbits--;
					...
					JButton eliminatedPiece = new JButton();
					eliminatedPiece.setPreferredSize(new Dimension(36, 36));
					eliminatedPiece.setEnabled(false);
					eliminatedPiece.setIcon(new ImageIcon(allPieces[x][y].getAnimal().getImage().getScaledInstance(36, 36,
					Image.SCALE_DEFAULT)));
					eliminatedPiece.setDisabledIcon(new ImageIcon(allPieces[x][y].getAnimal().getImage().getScaledInstance(36, 36,
					Image.SCALE_DEFAULT)));
					//resize an image icon by converting it to an image learned from
					//https://stackoverflow.com/questions/16343098/resize-a-picture-to-fit-a-jlabel/32885963#32885963
					if (allPieces[x][y].getColor().equals("gold"))
					{
						goldEliminatedContainer.add(eliminatedPiece);
					}
					else
					{
						silverEliminatedContainer.add(eliminatedPiece);
					}
          allPieces[x][y] = null;
	...
	//checks to see if a piece has any neighbors of the same color as itself
	...
	{
		if (playerState.equals("goldPlay") || playerState.equals("goldMove") || playerState.equals("goldPushPlace")	|| playerState.equals("goldPullPlace"))
		{
			for (int a = 0; a < goldPreviousPlays.size(); a++)
			{
				if (arraysEqual(goldPreviousPlays.get(a), allPieces) == true)
				{
					...
					//set the element at a particular index in an ArrayList to something - learned from
					...
					{
						goldLastPositionIndex = a; //this position is valid, so set this as the ending for this turn
						System.out.println("time still ok (gold)");
						System.out.println("Last Position Index (gold): " + goldLastPositionIndex);
						System.out.println("Times: " + goldTimesPreviouslyPlayed.get(goldLastPositionIndex));System.out.println("");  /*MAJOR BOOKMARK*/
						return false;
					}
				}
			}
		...
		else
		{
			for (int a = 0; a < silverPreviousPlays.size(); a++)
			{
				if (arraysEqual(silverPreviousPlays.get(a), allPieces) == true)
				{
					...
					{
						silverLastPositionIndex = a; //this position is valid, so set this as the ending for this turn
						System.out.println("time still ok (silver)");
						System.out.println("Last Position Index (silver): " + silverLastPositionIndex);
						System.out.println("Times: " + silverTimesPreviouslyPlayed.get(silverLastPositionIndex));
						System.out.println("");
						return false;
					}
				}
	...
	//return a spot on the board to its original color
  ...
  //checks the piece at each position in two arrays; if all of them are equal, return that the two arrays
	...
	public boolean arraysEqual (Piece[][] oldPosition, Piece[][] currentPosition)
	{
        ...
        if((oldPosition[a][b] != null && currentPosition[a][b] == null) || (oldPosition[a][b] == null &&
				currentPosition[a][b] != null))
        {
					//System.out.println("one null, one not");
          return false;
        }
        //get and compare color and power level since .equals doesn't work
        else if (oldPosition[a][b] != null && currentPosition[a][b] != null && !oldPosition[a][b].getColor()
				.equals(null) && !currentPosition[a][b].getColor().equals(null) && ((!oldPosition[a][b].getColor()
				.equals(currentPosition[a][b].getColor())) || 
				oldPosition[a][b].getPowerLevel() != currentPosition[a][b].getPowerLevel()))
				{
  ...
  public void addNewBoardPosition()
  {
	  ...
	  for (int a = 0; a < 8; a++) 
		{
          ...
          {
            copy[a][b] = new Piece(allPieces[a][b].getAnimal(), allPieces[a][b].getPowerLevel(),
						"goldSetup");
          }
          else
          {
		...
		if (playerState.equals("goldPlay") || playerState.equals("goldMove") || playerState.equals("goldPushPlace")
		|| playerState.equals("goldPullPlace"))
		{
			...
			System.out.println("Last Position Index (gold): " + goldLastPositionIndex)
			;System.out.println("Times: " + goldTimesPreviouslyPlayed.get(goldLastPositionIndex));
		}
		else
		{
	...
	//called at the start so the board position ArrayLists are not empty; /*adds a Piece[][] with >16 pieces
	//(so it will never match with any position in the middle of the game)
	//add the current ending board position to the ArrayList of past board positions for the current player*/
	//adds the starting position Piece[][] to both players' ArrayLists and sets their times played to 0
	//(since no player moved to create this position, it doesn't count against either of them).
  public void addDummyBoardPosition()
  {
	  ...
	  for (int a = 0; a < 8; a++) 
		...
		goldPreviousPlays.add(copy);
		//System.out.println("new position added");
		goldTimesPreviouslyPlayed.add(0);
		goldLastPositionIndex = goldPreviousPlays.size() - 1;
		silverPreviousPlays.add(copy);
		silverTimesPreviouslyPlayed.add(0);
		silverLastPositionIndex = silverPreviousPlays.size() - 1;
  }
	/*
	public void addDummyBoardPosition()
	  ...
	  for (int a = 0; a < allPieces.length; a++)
		{
				...
				{
					dummy[a][b] = allPieces[a][b];
				}
		}
		dummy[3][4] = new Piece(new ImageIcon("./grabbit.png"), 0, ""); //extra piece
  ...
  //for when a position has been created too many times. Resets the board to the last position before this
	//turn. 
	public void revertToLastBoardPosition()
	{
		...
		|| playerState.equals("goldPullPlace"))
		{
			lastBoardPosition = silverPreviousPlays.get(silverLastPositionIndex); //last turn was silver's
			for (int a = 0; a < allPieces.length; a++)
			{
				for (int b = 0; b < allPieces[0].length; b++)
				{
					allPieces[a][b] = lastBoardPosition[a][b];
          board[a][b].setIcon(lastBoardPosition[a][b].getAnimal());
          board[a][b].setDisabledIcon(lastBoardPosition[a][b].getAnimal());
				}
			}
		}
		else
		{
			lastBoardPosition = goldPreviousPlays.get(goldLastPositionIndex); //last turn was gold's
			for (int a = 0; a < allPieces.length; a++)
			{
				for (int b = 0; b < allPieces[0].length; b++)
				{
					if(lastBoardPosition[a][b] != null)
          {
            allPieces[a][b] = lastBoardPosition[a][b];
            board[a][b].setIcon(lastBoardPosition[a][b].getAnimal());
            board[a][b].setDisabledIcon(lastBoardPosition[a][b].getAnimal());
          ...
          else
          {
            ...
            board[a][b].setDisabledIcon(null);
          }
				}
			}
		}
